...
class OrderController {
    ...
    async getOrder(req, res) {
        const { _id } = req.user;
        try {
            const orders = await orderModel.find({ user: _id, deleted: false }).populate("user").populate("orderItems.productId").populate("orderItems.color");
            res.json({ orders });
        } catch (error) {
            throw new Error(error);
        }
    ...
    async getOrderUserById(req, res) {
        const { id } = req.params;
        try {
            const getOrderUserById = await orderModel.findById(id).populate("orderItems.productId").populate("orderItems.color");
